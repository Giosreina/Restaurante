...
public class FacadeAtender extends FacadeAbstract {
    ...
    private FacadeInventarioYAlmacen inventarioYAlmacen = new FacadeInventarioYAlmacen();
    ...
    public void atenderCliente() {
        while (estado.isEstado()) {
            ...
            while (true) {
                ...
                if (metodo.equals("SITIO") || metodo.equals("DOMICILIO") || metodo.equals("RAPPI")) {
                    break;
                }
            ...
            if (personalizar.equals("SI")) {
                ...
                terminal.imprimir("Desea hacer un batido personalizado o desea probar uno de nuestros batidos? (SI/NO)");
                String batidoPersonalisado = terminal.recibir();
                List<String> frutas = new ArrayList<>();
                if (batidoPersonalisado.equals("SI")) {
                    Batido batido = new Batido(pedido);
                    inventarioYAlmacen.mostrarContenidoAlmacen();
                    while (true) {
                        terminal.imprimir("Dame una fruta para agregar a tu batido");
                        frutas.add(terminal.recibir());
                        terminal.imprimir("Deseas añadir otra furta? (SI/NO)");
                        if (terminal.recibir().equals("NO")) {
                            batido.setFrutas(frutas);
                            break;
                        }
                    }
                    terminal.imprimir("Deseas azucar o lenche condensada? (1,2)");
                    if (terminal.recibir().equals("1")) {
                        batido.setAzucar(true);
                    } else {
                        batido.setLecheCondensada(true);
                    }
                } else {
                    // Builder de batidos
                    int batido = 0;
                    while (true) {
                        ...
                        try {
                            ...
                            if (batido >= 1 && batido <= 4) {
                                break;
                            } else {
                                terminal.imprimir("❌ Opción fuera de rango.");
                            }
                    ...
                    BatidoBuilder builder = switch (batido) {
                        case 1 ->
                            new BatidoRojoBuilder();
                        case 2 ->
                            new BatidoVerdeBuilder();
                        case 3 ->
                            new BatidoCitricoBuilder();
                        case 4 ->
                            new BatidoTropicalBuilder();
                        default ->
                            new BatidoRojoBuilder(); // nunca llega aquí, por seguridad
                    };

                    builder.setFrutas().setLecheCondensada().setAzucar();
                ...
                for (String a : adiciones) {
                    switch (a.trim()) {
                        ...
                        default ->
                            terminal.imprimir("⚠️ Adición desconocida: " + a.trim());
                    }
                }
